...
-- Author: 
-- Date: 2020-08-08
-- File: LuckCatData.lua
-- Module: LuckCatData
-- Description: Zhaocao data
------------------------------------------------
...
function LuckCatData:New(typeId)
...
function LuckCatData:ParseSelfCfgData(jsonTable)
...
function LuckCatData:ParsePlayerData(jsonTable)
...
-- Refresh data
function LuckCatData:RefreshData()
...
-- Check the red dots
function LuckCatData:CheckRedPoint()
...
function LuckCatData:ClearOnlineRedPoint()
    if self.ShowOnlineRedPoint then
...
function LuckCatData:AddLocalRecord(rate, count)
...
function LuckCatData:ReqChouJiang()
    if self.RemainCount <= 0 or self.CurCostCount >= self.CurLimitCount then
        Utils.ShowPromptByEnum("C_LUCKCAT_COUNT_NOTENOUGH")
        return
    end
    local _haveGold = GameCenter.ItemContianerSystem:GetEconomyWithType(ItemTypeCode.Gold)
    if _haveGold < self.CurCost then
        Utils.ShowPromptByEnum("C_LUCKCAT_YUANBAO_NOTENOUGH")
        return
    end
...
function LuckCatData:ResActivityDeal(jsonTable)
