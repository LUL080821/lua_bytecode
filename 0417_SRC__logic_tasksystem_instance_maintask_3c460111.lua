...
function MainTask:New(info)
    local _m = Utils.Extend(L_TaskBase:New(), self)
    _m:SetTask(info)
    return _m
end

function MainTask:SetTask(info)
    local _cfg = DataConfig.DataTask[info.modelId]
    if _cfg == nil then
    ...
    self.Data:SetData(info.modelId, TaskType.Main, 0, self:GetSubmitNpcId(info.modelId), true, _cfg.Type, _cfg.TaskName,
    ...
    if self:CanAutoRun() then
        ...
        if GameCenter.LuaTaskManager.IsAutoMainTask then
            GameCenter.TaskController:Pause()
        end
    end
    -- Main tasks need to be added to the task controller in advance
    GameCenter.TaskController:SetRunTimeTask(self)
    self:SetRecommendDes()
end

function MainTask:OnCanAutoRun()
    local _ret = false
    local _isAuto = false
    ...
    if GameCenter.LuaTaskManager.IsAutoMainTask and not GameCenter.BlockingUpPromptSystem:IsRunning() and _isAuto then
...
function MainTask:OnGetSubmitNpcId(id)
    local _ret = 0
    local _cfg = DataConfig.DataTask[id]
    if _cfg ~= nil then
...
function MainTask:OnUpdateTask(behavior)
    self:UpdateTargetDes()
end

function MainTask:OnGetTaskTarget(id)
    local _ret = ""
    local _cfg = DataConfig.DataTask[id]
    if _cfg ~= nil then
        _ret = _cfg.Target
...
function MainTask:OnGetTaskXZ(id)
    local _ret = ""
    local _cfg = DataConfig.DataTask[id]
    if _cfg ~= nil then
        _ret = _cfg.TaskXZ
...
function MainTask:OnGetActionName()
    return self.Data.Cfg.Animation
end

function MainTask:OnGetMovePos()
...
function MainTask:OnGetPlanShowEnter(id)
    local _ret = ""
    local _cfg = DataConfig.DataTask[id]
    if _cfg ~= nil then
...
function MainTask:OnGetNewType()
    return self.Data.Cfg.TargetType
end

function MainTask:OnGetUIDes(count, tCount)
    local _ret = ""
...
function MainTask:OnSubMitTaskOpenPanel()
    return self.Data.Cfg.OverTaskFunction
end

function MainTask:GetChuanDaoLimitData()
...
function MainTask:OnSetRecommendDes()
    ...
    if self.Data ~= nil then
        if self.Data.Cfg.RecommendTips ~= nil and self.Data.Cfg.RecommendTips ~= "" then
            ...
            if not GameCenter.LuaTaskManager:CanSubmitTask(self.Data.Id) then
                ...
                for i = 1, #_strs do
                    local _id = _strs[i]
                    local _cfg = DataConfig.DataTaskRecommendTips[_id]
                    if _cfg ~= nil then
...
function MainTask:OnUpdateTargetDes()
    if self.Data.Cfg.RecommendTips ~= nil and self.Data.Cfg.RecommendTips ~= "" then
        if not GameCenter.LuaTaskManager:CanSubmitTask(self.Data.Id) then
            local _isFind = false;
...
function MainTask:OnJoinRecommendDaily()
    local _ret = false
    -- If the task is not completed
    if not GameCenter.LuaTaskManager:CanSubmitTask(self.Data.Id) then
        for i = 1, #self.Data.RecommendList do
...
function MainTask:OnIsAuto()
    return self.Data.Cfg.IsAuto == 0
end

function MainTask:OnGetJinDuDes(count, tCount)
    local _ret = ""
    if self:GetVariable() == 1 then
        _ret = UIUtils.CSFormat(self.Data.Cfg.ConditionsDescribe, CommonUtils.GetLevelDesc(count), CommonUtils.GetLevelDesc(tCount))
    else
...
function MainTask:OnIsShowRecommendUI()
    local _ret = false
    if self.Data ~= nil then
...
function MainTask:OnGetVariable()
    local _ret = 0
...
function MainTask:OnGetFollowModel()
    local _ret = ""
    if self.Data.Cfg ~= nil then
