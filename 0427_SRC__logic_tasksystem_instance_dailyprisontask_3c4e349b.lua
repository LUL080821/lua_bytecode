...
-- Author: Wang Sheng
-- Date: 2021-03-19
-- File: DailyPrisonTask.lua
-- Module: DailyPrisonTask
-- Description: Prison Daily Task Example
------------------------------------------------
-- Quote
local L_TaskBase = require "Logic.TaskSystem.Instance.TaskBase"
local DailyPrisonTask = {}

function DailyPrisonTask:New(info, isAccess)
    local _m = Utils.Extend(L_TaskBase:New(), self)
    _m:SetTask(info, isAccess)
    return _m
end

function DailyPrisonTask:SetTask(info, isAccess)
    if info == nil then
        return
    end
    ...
    local _cfg = DataConfig.DataTaskDailyPrison[info.modelId]
    if _cfg == nil then
        Debug.LogError("DailyPrisonTask Id Is Error");
        return
    end
    ...
    self.Data:SetData(info.modelId, TaskType.DailyPrison, self:GetAccessNpcId(info.modelId),
    ...
    self.Data.SubType = _cfg.DailyPrisonSubtype
    if _cfg.DailyPrisonSubtype == 0 then
        self.Data.Sort = TaskSort.DailyExp
    elseif _cfg.DailyPrisonSubtype == 1 then
        Data.Sort = TaskSort.DailyGold
    end
    self.Data.StarNum = info.star
    self.Data.CurStep = info.count
    self.Data.Cfg = _cfg
    local _globCfg = DataConfig.DataGlobal[GlobalName.DailyPrisonTaskMax]
    local _strs = Utils.SplitStr(_globCfg.Params, ';')
    local _values = nil
    if #_strs > _cfg.DailyPrisonSubtype then
        _values = Utils.SplitNumber(_strs[_cfg.DailyPrisonSubtype + 1], '_')
    end
    ...
    if isAccess then
        if GameCenter.LuaTaskManager:IsInNotRecieveContainer(info.modelId) then
            ...
            if _behavior.Type == TaskBeHaviorType.PassCopy then
                if GameCenter.LuaTaskManager.IsAutoDailyPrisonTask then
                    _behavior:DoBehavior()
                end
            else
                if GameCenter.LuaTaskManager.IsAutoDailyPrisonTask and _behavior.Type ~= TaskBeHaviorType.OpenUI then
                    _behavior:DoBehavior()
                end
            end
...
function DailyPrisonTask:GetRwardStr(cfg, starNum)
    local _reward_Str = nil
    if cfg ~= nil then
        if starNum == DailyPrisonTaskStarNum.Star_0 then
            _reward_Str = cfg.Rewards0
        elseif starNum == DailyPrisonTaskStarNum.Star_1 then
            _reward_Str = cfg.Rewards1
        elseif starNum == DailyPrisonTaskStarNum.Star_2 then
            _reward_Str = cfg.Rewards2
        elseif starNum == DailyPrisonTaskStarNum.Star_3 then
            _reward_Str = cfg.Rewards3
        elseif starNum == DailyPrisonTaskStarNum.Star_4 then
            _reward_Str = cfg.Rewards4
        elseif starNum == DailyPrisonTaskStarNum.Star_5 then
...
function DailyPrisonTask:OnGetAccessNpcId(id)
    local _ret = 0
    local _cfg = DataConfig.DataTaskDailyPrison[id]
    if _cfg ~= nil then
...
function DailyPrisonTask:OnGetSubmitNpcId(id)
    local _ret = 0
    local _cfg = DataConfig.DataTaskDailyPrison[id]
    if _cfg ~= nil then
        _ret = _cfg.OverNpc
...
function DailyPrisonTask:OnUpdateTask(behavior)
...
function DailyPrisonTask:OnGetTaskTarget(id)
    local _ret = ""
    local _cfg = DataConfig.DataTaskDailyPrison[id]
    if _cfg ~= nil then
        _ret = _cfg.GoalNpc
...
function DailyPrisonTask:OnGetTaskXZ(id)
    local _ret = ""
    local _cfg = DataConfig.DataTaskDailyPrison[id]
    if _cfg ~= nil then
        _ret = _cfg.TaskXZ
...
function DailyPrisonTask:OnGetPlanShowEnter(id)
    local _ret = ""
    local _cfg = DataConfig.DataTaskDailyPrison[id]
    if _cfg ~= nil then
