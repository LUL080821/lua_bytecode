...
function KillMonsterTrainMap:New(type, taskId)
    return Utils.Extend(L_Behavior:New(type, taskId), self)
end

function KillMonsterTrainMap:OnSetTarget(id, count, talkId, x, y, itemID, type)
    if type == nil then
        type = -1
    end
...
function KillMonsterTrainMap:OnSetTargetDes()
    local _task = GameCenter.LuaTaskManager.TaskContainer:FindTakByID(self.Id)
    if _task == nil then
        return
    end
...
function KillMonsterTrainMap:OnCanTransPort()
    local _ret = true
...
function KillMonsterTrainMap:CallBack()
...
function KillMonsterTrainMap:OnDoBehavior(isClick)
    -- Debug.Log("yy KillMonsterTrainMap  OnDoBehavior   ")
    self.IsClick = isClick
    self:TransPort(Utils.Handler(self.CallBack, self))
end

return KillMonsterTrainMap
