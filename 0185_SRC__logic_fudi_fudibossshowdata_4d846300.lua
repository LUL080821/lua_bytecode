...
-- Author:
-- Date: 2019-05-15
-- File: FuDiBossShowData.lua
-- Module: FuDiBossShowData
-- Description: Fudi boss display data
------------------------------------------------
-- Quote
...
function FuDiBossShowData:New(msg)
    ...
    DataConfig.DataGuildBattleBoss:Foreach(function(k, v)
        local list = nil
...
function FuDiBossShowData:SetData(msg)
    ...
    --ListGuanZhu
    if msg.attentionList ~= nil then
        for i = 1,#msg.attentionList do
            GameCenter.FuDiSystem.ListGuanZhu:Add(msg.attentionList[i])
        end
    end
end
