...
-- author:
-- Date: 2020-02-10
-- File: UIExpCopyMapPanel.lua
-- Module: UIExpCopyMapPanel
-- Description: Pagination of experience copy
------------------------------------------------

local L_UICopyMapLevelSelect = require "UI.Forms.UICopyMapForm.UICopyMapLevelSelect"

-- //Module definition
local UIExpCopyMapPanel = {
    -- Current transform
    Trans = nil,
    Go = nil,
    -- father
...
function UIExpCopyMapPanel:OnFirstShow(trans, parent, rootForm)
    self.Trans = trans;
    ...
    self.MegreNeedLevelValue = DataConfig.DataDaily[8].SweepLevel
    ...
    self.ExpAddValue = UIUtils.FindLabel(self.Trans, "Center/ExpAdd");
    self.ExpAddBtn = UIUtils.FindBtn(self.Trans, "Center/ExpAddBtn");
    -- UIUtils.AddBtnEvent(self.ExpAddBtn, self.OnExpAddBtnClick, self);
    UIUtils.SetBtnState(self.ExpAddBtn.transform, false)

    return self;
end

function UIExpCopyMapPanel:Show()
    -- Play the start-up picture
    self.AnimModule:PlayEnableAnimation();
    self.CurCopyData = GameCenter.CopyMapSystem:FindCopyData(GameCenter.CopyMapSystem.ExpCopyID);
    self:RefreshPage(true);
end

function UIExpCopyMapPanel:Hide()
    -- Play Close animation
    self.Go:SetActive(false);
end

function UIExpCopyMapPanel:RefreshDet(cfg)
    ...
    local _drugExpAdd = GameCenter.OfflineOnHookSystem:GetMainFormShowExpRate(OnHookExpAddType.Drug);
    if _drugExpAdd <= 0 then
        UIUtils.SetTextByEnum(self.ExpAddValue, "C_EXP_COPY_NOTUSE")
    else
        UIUtils.SetTextByEnum(self.ExpAddValue, "Percent", math.floor(_drugExpAdd));
    end
end

-- Refresh the interface
function UIExpCopyMapPanel:RefreshPage(rePos)
    if self.CurCopyData == nil then
        return
    end
...
function UIExpCopyMapPanel:OnAddCountBtnClick()
    if self.CurCopyData == nil then
        return;
    end
...
-- Enter button click
function UIExpCopyMapPanel:OnEnterBtnClick()
    if self.CurCopyData == nil then
        return;
    end
    ...
    elseif (self.CurCopyData.FreeCount + self.CurCopyData.VIPCount) <= 0 and self.CurCopyData.CanBuyCount <= 0 then
        ...
        if self.CurCopyData.CurMergeCount >= 2 then
            ...
            if _haveCount < self.CurCopyData.CurMergeCount - 1 then
                -- Pop-up item acquisition prompt
        ...
        GameCenter.BISystem:ReqClickEvent(BiIdCode.LYYTChallenge);
    end
end

-- Efficiency button click
function UIExpCopyMapPanel:OnExpAddBtnClick()
    -- GameCenter.PushFixEvent(UIEventDefine.UIExpCopyDoubleItemForm_OPEN)
    Debug.Log("Disabled UIExpCopyDoubleItemForm 2")
end

function UIExpCopyMapPanel:RefreshMegreCount()
    local _allCount = self.CurCopyData.JionCount + self.CurCopyData.FreeCount + self.CurCopyData.VIPCount;
...
-- Number of merges
function UIExpCopyMapPanel:MergeCountBtnClick()
    local _lpLevel = GameCenter.GameSceneSystem:GetLocalPlayerLevel()
    if _lpLevel < self.MegreNeedLevelValue then
...
function UIExpCopyMapPanel:Update(dt)
    self.SeleclPanel:Update(dt)
end

return UIExpCopyMapPanel;
