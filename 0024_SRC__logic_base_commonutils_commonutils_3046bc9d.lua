...
-- Author:
-- Date: 2019-05-24
-- File: CommonUtils.lua
-- Module: CommonUtils
-- Description: Commonly used tool classes--This collection of some common logical methods.
------------------------------------------------
local CSCommonUtils = CS.Thousandto.Code.Logic.CommonUtils

local CommonUtils = {
}
local L_DianFeng_Lv = 360
...
-- Get the icon name of the player's avatar
function CommonUtils.GetPlayerHeaderSpriteName(headid,occ)
    return CSCommonUtils.GetPlayerHeaderSpriteName(headid,occ)
end

-- Get the icon name of the player's avatar frame
function CommonUtils.GetPlayerHeaderFrameSpriteName(headframeid,occ)
    -- return CSCommonUtils.GetPlayerHeaderFrameSpriteName(headframeid,occ)
    return "head_bg"
end

-- Get the background of the level SpriteName
...
function CommonUtils.CovertToBigUnit(num, validCount)
    if not num then
        return ""
    end
    return CSCommonUtils.CovertToBigUnit(num, validCount or 0)
end
